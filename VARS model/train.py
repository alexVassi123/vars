...
def trainer(train_loader,
    ...
    run_dir = os.path.join(model_name, run_timestamp)
    os.makedirs(run_dir, exist_ok=True)
    print(f"Saving predictions to: {run_dir}")

    for epoch in range(epoch_start, max_epochs):

        print(f"Epoch {epoch+1}/{max_epochs}")

        # Log current epoch to file
        with open(os.path.join(run_dir, "progress.txt"), "w") as f:
            f.write(f"epoch: {epoch+1}/{max_epochs}\nstarted: {run_timestamp}\nupdated: {datetime.now().strftime('%Y-%m-%d_%H-%M')}\n")
        ...
        # Save checkpoint every epoch after warmup
        if counter > 3:
            state = {
            'epoch': epoch + 1,
        ...
        if val_lb > best_val_lb:
            ...
            print(f"New best val LB: {best_val_lb:.2f} at epoch {epoch+1}")
        else:
            epochs_without_improvement += 1
            print(f"No val improvement for {epochs_without_improvement}/{patience} epochs (best: {best_val_lb:.2f})")

        if epochs_without_improvement >= patience:
            print(f"Early stopping triggered at epoch {epoch+1}. Best val LB: {best_val_lb:.2f}")
            break
...
def train(dataloader,
    ...
    # Disable autograd during eval to free activation memory (val/test use 4 views).
    eval_ctx = torch.no_grad() if not train else torch.enable_grad()
    with eval_ctx:
    ...
    with open(os.path.join(output_dir, prediction_file), "w") as outfile:
        json.dump(data, outfile)
    return os.path.join(output_dir, prediction_file), loss_total_action / total_loss, loss_total_offence_severity / total_loss
