...
def checkArguments():
    ...
    # args.pooling_type
    if args.pooling_type not in ['max', 'mean', 'attention', 'transformer', 'cross_attention']:
        print("Could not find your desired argument for --args.pooling_type:")
        print("Possible arguments are: max, mean, attention, transformer, cross_attention")
        exit()
...
def main(*args):
    ...
    if only_evaluation == 0:
        ...
        train_loader = torch.utils.data.DataLoader(dataset_Train,
            ...
            num_workers=max_num_worker, pin_memory=True,
            persistent_workers=(max_num_worker > 0), prefetch_factor=4)
    ...
    if only_evaluation == 0:
        ...
        trainer(train_loader, val_loader2, test_loader2, model, optimizer, scheduler, criterion,
                best_model_path, epoch_start, model_name=model_name, path_dataset=path, max_epochs=max_epochs, patience=patience)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--weight_decay", required=False, type=float, default=0.01, help="Weight decay")
    parser.add_argument("--patience", required=False, type=int, default=15, help="Early stopping patience (epochs without val improvement)")
    ...
    parser.add_argument("--seed", required=False, type=int, default=42, help="Random seed for reproducibility / seed ensembling")
